...
Добавлено кэширование запросов в файлы к АПИ Новой почты. Кэш нужен для разработки приложения, чтобы в процессе каждый раз не обращаться к АПИ, а брать ответ из кэша (особенно актуально для тяжеловесных справочников). 

      Config::setCacheStatus(true); // or false
      Config::setCacheFilePath('path'); // путь к дирректории с файлами кэша
      Config::setPrefix('prefix'); // присатвка перед именем файла кэша
      Config::setCacheTime($value); // время хранения кэша в сек., перед обновлением
 
 Кширование в этой версии включается для всех запросов и только в методе _send() класса Core/SendData
 В планах:
 -отключение кэша для запросов записи данных в Новую Почту
 -записывать названия файлов кэша в зависимости от дополнительных параметров запроса
 -добавить кэш в метод _getResponseBatch()



# Новая Почта. SDK для личного кабинета API 2 Добавлен кэш
